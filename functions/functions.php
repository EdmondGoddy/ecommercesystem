...
function add_cart(){
  ...
  if (isset($_POST["cart"])) { //add_cart
    ...
    $check_product = "SELECT * FROM cart WHERE ((ip_add='$ip_add' AND session_id='$sess') AND p_id='$p_id') ";
    ...
    if (mysqli_num_rows($run_check) > 0) {
     ...
     // }
      ...
      //echo "<script>window.location('details.php?pro_id=$p_id', '_self')</script>";
      // OR echo "<script>windows.location('details.php?pro_id=$p_id')</script>";
      //header("location:details.php?pro_id=$p_id");
    }else{


      $query = "INSERT INTO cart (session_id, p_id, ip_add, qty, size, price) VALUES ('$sess', '$p_id', '$ip_add', '$product_qty', '$product_size', '$product_price')";
...
function items(){
  ...
  $get_items = "SELECT * FROM cart WHERE ip_add='$ip_add'  AND session_id='$sess' ";
